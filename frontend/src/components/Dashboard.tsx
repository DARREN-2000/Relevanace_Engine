...
export const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get("http://localhost:8000/api/analytics/fatigue")
    ...
    axios
      .get("http://localhost:8000/api/analytics/channels")
      .then((res) => setChannels(res.data))
      .catch(() => {
        setChannels([
          {
            channel: "Email",
            total_decisions: 300000,
            suppressed: 50000,
            open_rate: 0.25,
            click_rate: 0.1,
          },
    ...
    axios
      .get("http://localhost:8000/api/analytics/suppression")
    ...
    >
      <div style={{ marginBottom: "30px" }}>
        ...
        <p style={{ color: "#a1a1aa", margin: 0 }}>
          Real-time metrics from the Consentinel Engine.
        </p>
      </div>
      ...
      >
        <div className="glass-panel" style={statCardStyle}>
          ...
          <p style={statValueStyle}>
            {summary.total_users ? formatNumber(summary.total_users) : "-"}
          </p>
        </div>
        <div className="glass-panel" style={statCardStyle}>
          ...
          <p style={statValueStyle}>
            {summary.total_decisions
              ? formatNumber(summary.total_decisions)
              : "-"}
          </p>
        </div>
        <div className="glass-panel" style={statCardStyle}>
          ...
          <p style={statValueStyle}>
            {summary.suppression_rate !== undefined
              ? (summary.suppression_rate * 100).toFixed(1) + "%"
              : "-"}
          </p>
        </div>
        <div className="glass-panel" style={statCardStyle}>
          ...
          <p style={statValueStyle}>
            {summary.avg_fatigue_score !== undefined
              ? summary.avg_fatigue_score.toFixed(2)
              : "-"}
          </p>
        </div>
      </div>
                ...
                >
                  {suppression.map((_, index) => (
                    <Cell
                      key={`cell-${index}`}
                      fill={
                        ["#ef4444", "#f59e0b", "#3b82f6", "#8b5cf6"][index % 4]
                      }
                    />
                  ))}
                </Bar>
